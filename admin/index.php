...
<html ng-app="myTimesheet" xmlns="http://www.w3.org/1999/xhtml">
<head>
									 ...
									 if(val <= 8){
										 if(event.which == 13)
										   { 
									  		...
									  		$.ajax({    type: "POST",
												        url:"config.php",
														data: "value="+$('.ajax input').val()+"&rownum="+arr[2]+"&field="+arr[1]+"&day="+arr[3],
														success: function(data){
															 $('.ajax').html($('.ajax input').val());
								                             $('.ajax').removeClass('ajax');
														}});
...
<th></th>
   ...
   //  		$weekends = date('N', strtotime($i));
			...
			if(isweekend(date('o'),date("m"),$i) == 1){
...
<?php
...

	{
		if($alt == 1)
		{
		   echo "<tr ng-controller='MyCtrl' class='{$rows["project_id"]}' >";
		   $alt = 0; 	
		}
		else
		{
		   echo "<tr ng-controller='MyCtrl' class='{$rows["project_id"]} alt'>";
		   $alt = 1;
		}
        	...
        	// }	
				...
				// if($rows["project_id"] == $p_id[$cc]){
					// if($rows["project_id"] == $p_id[$cc]){
						...
						echo "<td id = 'hours' ng-grid='gridOptions' class = 'edit hour gridStyle ".$rows["project_id"]." {$i}'>".$hours[$cc]."</td>";
							...
							for (var ii = 1; ii <= <?php echo $nr_days ?>; ii++) {
								...
								$('th.weekend').each(function(index,value){
									var a = $(this).html();
										...
										$("td.hour."+ii+"."+a).eq($('table tr th').index()).removeClass('edit').removeClass('hour').removeClass().addClass('weekend').html("0");
